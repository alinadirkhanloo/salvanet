...
export class ArticleEditComponent {
  ...
  constructor(
    ...
    this.editForm = this._formBuilder.group({
      ...
      isForeign: ['', [Validators.required, Validators.maxLength(36)]],
      id:-1
    });
  }
  ...
  loadThesis(id:number|string) {
    let res = this.articleService.getById(id).subscribe({
      next:(result)=>{
  ...
  submit() {
    ...
    if (this.editForm.valid) {
      let rest = this.updateMode?this.articleService.insert(this.editForm.value) : this.articleService.update(this.editForm.value.id,this.editForm.value);
